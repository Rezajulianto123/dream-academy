...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error?.message || 'Registrasi gagal. Silakan coba lagi.');
      }
    ...
    <div className="flex min-h-screen items-center justify-center p-4 bg-slate-50">
      <div className="w-full max-w-md bg-white rounded-xl shadow-sm border border-slate-200 p-8 space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Daftar Akun Baru</h1>
          <p className="text-sm text-slate-500">Mulai belajar speaking bahasa Inggris sekarang</p>
        </div>
